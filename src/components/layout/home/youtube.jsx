...
//          /> :
//          <>
//           <iframe width="240px" height="130px" src={`${video.path}?autoplay=0&controls=0&disablekb=1`} onLoad={() =>setLoading(false)} style={{border:'2px solid white'}}></iframe>  
...
import { IconButton, Link, Skeleton, Typography, useMediaQuery, useTheme } from '@mui/material';
 ...
 return (
  ...
  >
    ...
    <defs>
     ...
     <div className="slider-container relative px-14 cursor-pointer " style={{marginBlockEnd:'35px', maxWidth:' 1200px',margin: 'auto',paddingBlockEnd:"30px"}}>
     <Slider {...settings} className='w-full'>
       {tnVideoUrl.map((video, index) => (
         <div key={index} className='pr-4' style={{padding:"0px 20px"}}>
           {loading ? (
           ...
           <Typography variant='h6' fontWeight={600} className='pt-3 text-white' onClick={(e) => handleOpen(e,video)} sx={{cursor:"pointer"}}>
             {video.name}
           </Typography>
         </div>
       ))}
